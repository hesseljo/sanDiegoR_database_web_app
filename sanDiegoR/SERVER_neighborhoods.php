    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
      <div class="container">
        ...
        <div class="row">
          ...
          if(!($stmt = $mysqli->prepare("INSERT INTO neighborhoods(name, per_capita_income) VALUES (?,?)"))){
            echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
          }
          if(!($stmt->bind_param("ss",
                        $_POST['name'],
                        $_POST['per_capita_income'])))  {
            echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
          }
          if(!$stmt->execute()){
            echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
          } else {
            echo "** Added " . $stmt->affected_rows . " rows to neighborhoods **";
          }
          ?>

          <form action="USER_neighborhoods.php">
